...
namespace Movieplayingwebsite.Repositories.Implement
{
    public class MovieService : IMovieService
    {
        ...
        public bool Delete(int id)
        {
            try
            {
                ...
                {
                    ctx.MovieGenre.Remove(movieGenre);//removing from moviegenre table
                }
        ...
        public MovieListVm List(string term="", bool paging=false, int currentPage=0)
            //paging flag is created because we are not using paging in admin pannel
        {
            ...
            if (paging)
            {
                //paging will be applied
                int pageSize = 5;//5 records on index page
        ...
        public bool Update(Movie model)
        {
            try
            {
                var genresToBeDeleted = ctx.MovieGenre.Where(a => a.MovieId == model.Id && !model.Genres.Contains(a.GenreId)).ToList();
